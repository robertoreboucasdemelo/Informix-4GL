###############################################################################
# Nome do Modulo: bdatq016                                           Wagner   #
...
#--------------------------------------------------------------------
 ...
 declare c_bdatq016_sitrecl cursor with hold for select_sitrecl
...
#--------------------------------------------------------------------
 ...
 declare c_bdatq016_ligrecl cursor with hold for select_ligrecl
...
#--------------------------------------------------------------------
 ...
 declare c_bdatq016_assunto cursor with hold for select_assunto
...
#--------------------------------------------------------------------
 ...
 declare c_bdatq016_dominio cursor with hold for select_dominio
...
#--------------------------------------------------------------------
 ...
 declare c_bdatq016_ligapol cursor with hold for select_ligapol
...
let ws.datter = ws.datini + 1 units month - 1 units day
 ...
 foreach c_bdatq016  into ws.lignum , ws.rclrlzdat

    #-----------------------------
    # servico/assunto       
    #-----------------------------
    open  c_bdatq016_ligrecl  using  ws.lignum
    fetch c_bdatq016_ligrecl  into   ws.c24astcod, 
                                     ws.atdsrvnum,
                                     ws.atdsrvano
    close c_bdatq016_ligrecl
    ...
    #-----------------------------
    open  c_bdatq016_assunto  using  ws.c24astcod
    fetch c_bdatq016_assunto  into   ws.c24astdes
    close c_bdatq016_assunto
    ...
    # Ultima situacao
    #-----------------------------
    open  c_bdatq016_sitrecl using ws.lignum, ws.lignum
    fetch c_bdatq016_sitrecl into  ws.c24rclsitcod, ws.rclrlzdat_a
    close c_bdatq016_sitrecl

    #-----------------------------
    # descricao do situacao  
    #-----------------------------
    open  c_bdatq016_dominio  using  ws.c24rclsitcod
    fetch c_bdatq016_dominio  into   ws.rclsitdes
    close c_bdatq016_dominio

    #-----------------------------
    # apolice  
    #-----------------------------
    open  c_bdatq016_ligapol using  ws.lignum        
    fetch c_bdatq016_ligapol  into  ws.succod,   ws.ramcod,
                                    ws.aplnumdig,ws.itmnumdig
    close c_bdatq016_ligapol 
 ...
 unload to "/adat/recllist.txt" select * from tbtemp 

 #------------------------------------------------------------------
 # E-MAIL PORTO SOCORRO
 #------------------------------------------------------------------
 #let ws.run =  "mailx -r 'Socorro_Porto/spaulo_psocorro_controles@u23'",
 #              " -s 'Reclamacoes_mes' ",
