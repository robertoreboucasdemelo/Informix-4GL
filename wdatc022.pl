...
function funcchama()
{
  var myemis = "/j2ee/atendcentral/jdatc014.do?usrtip=$usrtip&webusrcod=$webusrcod&sesnum=$sesnum&macsissgl=$macsissgl";
  location = myemis;
...
END
 ...
 #-----------------------------------------------------------------------------
$prog = "(cd /webserver/cgi-bin/ct24h/trs ; . ../../setvaru01.sh; /webserver/cgi-bin/ct24h/trs/wdatc023.4gc $ARG 2>&1)";
...
### $prog = "(cd /webserver/cgi-bin/ct24h/trs ; . ../../setvaru01.sh; wdatc015.4gc $ARG 2>&1)";
###
### open (PROG, "$ambiente\"$prog\" |") || die ("N\343o foi poss\355vel executar $prog: $!\n");
###
### #-----------------------------------------------------------------------------
...
### $texto    = 'Consulte os serviços pendentes. O sistema é atualizado a cada 30 segundos. Caso não clique sobre o serviço, automaticamente será repassado a outro prestador. Veja no final da tela se existem complementos de serviço.';
 ...
 start_form(-method=>'POST',
            -name=>'wdatc022',
            -action=>'/webserver/cgi-bin/ct24h/trs/wdatc022.pl?usrtip=%s&webusrcod=%d$sesnum=%d&macsissgl=%s&atdsrvnum=%d&atdsrvano=%d&caddat=%s&cadhor=%s&,$usrtip,$webusrcod,$sesnum,$macsissgl,$atdsrvnum,$atdsrvano,$caddat,$cadhor'
           ),"\n";
...
"     location.replace('/j2ee/atendcentral/jdatc014.do?usrtip=$usrtip&webusrcod=$webusrcod&sesnum=$sesnum&macsissgl=$macsissgl');",
"</script>",
