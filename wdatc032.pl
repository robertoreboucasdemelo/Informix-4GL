...
"\n",
 ...
 #-----------------------------------------------------------------------------
 $texto = 'Quando precisar manter contato com sua(s) viatura(s), ou com todas simultaneamente, auxiliando no controle geral da operação, envie uma mensagem. Informe o texto desejado no campo "Mensagem" e clique no botão "enviar".';
...
function func_checa()
{
     ...
     if (document.wdatc032.mensagem.value.length < 1)
       ...
       if (document.wdatc032.mensagem.value.length > 360)
         {
          v_aux += "Mensagem está com " + document.wdatc032.mensagem.value.length + " caracteres. Tamanho máximo deve ser de 360 caracteres!";
         }
       }
