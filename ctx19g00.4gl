...
display " argumentos.prpnumdig    ", argumentos.prpnumdig
 ...
 if argumentos.c24astcod = "N10" or
    argumentos.c24astcod = "N11" then
    display "<ctx19g00> **** Gravando Histórico da Ligação"
    #---------------------------------------------------------------------------
...
#---------------------------------------------------------------------------
function ctx19g00_consiste(l_par_cons) ## Consistir parametros obrigatórios
#---------------------------------------------------------------------------
       ...
       if l_erro = 0 then
           ...
           whenever error stop
             ...
             if l_exist = false then
                ...
                if sqlca.sqlcode <> 0 then
                   let l_erro = sqlca.sqlcode
                   let l_msg = " Erro ao selecionar ligação de B16  < ",l_erro , "> ",lr_trn.trnlignum , " > "
                   display " Erro ao selecionar ligação de B16  < ",l_erro , "> ",lr_trn.trnlignum , " > "
                end if
           ...
           if l_erro = 0 then
              if lr_trn.necgerlignum is not null then
                 ...
                 if l_exist = false then
                    ...
                    if sqlca.sqlcode <> 0 then
                       let l_erro = sqlca.sqlcode
                       let l_msg = " Erro ao selecionar ligação de Transferencia  < ",l_erro, ">" ,lr_trn.necgerlignum , " > "
                       display " Erro ao selecionar ligação de Transferencia  < ",l_erro, ">" ,lr_trn.necgerlignum , " > "
                    end if
...
function ctx19g00_valida_atd_trn(lr_atdnum)
   ...
   if sqlca.sqlcode <> 0 then
      if sqlca.sqlcode = 100 then
        let l_msg = "Atendimento não disponivel para transferencia"
