...
#------------------
   ...
   define lr_restart      record
          chave           char(30)
   end record

   define lr_recarga      record
          succod          like abamapol.succod
         ,aplnumdig       like abamapol.aplnumdig
         ,emsdat          like abamdoc.emsdat
   ...
   define i             smallint
   define l_itens       dec(9,0)

   define l_srvnom char(09)

   initialize lr_abamapol.*         to null
   initialize lr_abbmdoc_1.*        to null
   ...
   initialize lr_restart.*          to null
   initialize lr_recarga.*          to null
...
#   prepare p_ins_abamapol from l_sql
   ...
   let l_sql = " insert into porto@",m_hostname clipped,":abbmapp_atd (succod     ",
                                   " ,aplnumdig  ",
                                   " ,itmnumdig  ",
                                   " ,dctnumseq  ",
                                   " ,rsgflg     ",
...
#   prepare p_ins_abbmitem from l_sql
   ...
   let l_sql = ' delete from porto@',m_hostname clipped,':igbmparam where relsgl = ? and relpamseq = ? '
   prepare p_dlt_igbmparam01 from l_sql
...
#   prepare p_dlt_abamcor01 from l_sql
   ...
   let l_sql = ' delete from porto@',m_hostname clipped ,':abcmaces where succod = ? and aplnumdig = ? and itmnumdig = ? and assnum = ? '
   prepare p_dlt_abcmaces01 from l_sql

   #let l_sql = ' delete from porto@',l_srvnom ,':abbmveic where succod = ? and aplnumdig = ? and itmnumdig = ? and dctnumseq = ? '
   #prepare p_dlt_abbmveic01 from l_sql
   ...
   let l_sql = ' select relpamtxt[01,02] '
              ,'       ,relpamtxt[03,11] '
              ,'       ,relsgl '
              ,'       ,relpamseq '
              ,'   from porto@',m_hostname clipped,':igbmparam '
              ,' where relsgl in ("ct24h_bdatm003","ct24h_bdata129","ct24h_bdata126")'
   prepare p_igbmparam_01 from l_sql
   declare c_igbmparam_01 cursor with hold for p_igbmparam_01
   let l_sql = " select max(relpamseq) "                        #Johnny,Biz
              ,"   from porto@",m_hostname clipped,":igbmparam"
              ,"  where relsgl = 'ct24h_bdata129' "
   prepare pbdata129001 from l_sql
   declare cbdata129001 cursor with hold for pbdata129001
   let l_sql = " insert into porto@",m_hostname clipped,":igbmparam"
              ," (relsgl,relpamseq,relpamtip,relpamtxt) "
              ," values('ct24h_bdata129',?,1,?)         "
   prepare pbdata129002 from l_sql
   ...
   while true
      ...
      open c_igbmparam_01
      foreach c_igbmparam_01 into lr_recarga.succod
                                 ,lr_recarga.aplnumdig
                                 ,lr_recarga.relsgl
                                 ,lr_recarga.relpamseq

         open c_abamdoc_recarg using lr_recarga.succod
                                    ,lr_recarga.aplnumdig
         foreach c_abamdoc_recarg into lr_abamdoc_rec.succod
                                      ,lr_abamdoc_rec.aplnumdig

            display ''
            display 'REGISTRO SENDO PROCESSADO - ' ,lr_recarga.succod clipped ,'/' ,lr_recarga.aplnumdig
            display ''
            ...
            if sqlca.sqlcode = notfound then
               ...
               let l_dpz_flg = 1
               rollback work
               call bdata129_erro(lr_abamdoc_rec.succod
                                 ,lr_abamdoc_rec.aplnumdig)
               continue foreach
            end if
            ...
            foreach c_abbmdoc02 into lr_abbmdoc.*
                ...
                #      let l_dpz_flg = 1
                #      rollback work
                #      call bdata129_erro(lr_abamapol.succod
                #                     ,lr_abamapol.aplnumdig)
                #      exit foreach
                #   end if
                #end if
                ...
                #   #--------------------
                #   whenever error continue
                #   execute p_dlt_abbmveic01 using lr_abamapol.succod
                #                                 ,lr_abamapol.aplnumdig
                #                                 ,lr_abbmveic.itmnumdig
                #                                 ,lr_abbmveic.dctnumseq
                #   whenever error stop
                #
                #   if sqlca.sqlcode <> 0 then
                #      display '#--> Problema ao deletar tabela abbmveic. Erro ' ,sqlca.sqlcode clipped,'/' ,sqlca.sqlerrd[2] clipped ,' registro: ' ,l_lidos
                #             ,' Chave desprezada Suc/Apol - ' ,lr_abamapol.succod ,'/' ,lr_abamapol.aplnumdig
                #      let l_desprezados = l_desprezados + 1
                #      let l_dpz_flg = 1
                #      rollback work
...
#                end if
         ...
         #---> Limpa chave igbmparam
         whenever error continue
         execute p_dlt_igbmparam01 using lr_recarga.relsgl
                                        ,lr_recarga.relpamseq
         whenever error stop

         if sqlca.sqlcode <> 0 then
            display '#--> Problema ao deletar tabela igbmparam. Erro ' ,sqlca.sqlcode clipped,'/' ,sqlca.sqlerrd[2] clipped
                   ,' Chave desprezada resgl/relpamseq - ' ,lr_recarga.relsgl ,'/' ,lr_recarga.relpamseq
         end if
...
open cbdata129001
whenever error continue
fetch cbdata129001 into l_relpamseq
whenever error stop
if sqlca.sqlcode <> 0 then
	 display "Problemas ao acesso do cursor cbdata129001.Erro.:"
end if
...
#    where relsgl = 'ct24h_bdata129'
   ...
   for l_i = 1 to 10
      ...
      execute pbdata129002 using l_relpamseq
                             ,l_relpamtxt
      whenever error stop
      # insert into porto@u18w:igbmparam (relsgl,relpamseq,relpamtip,relpamtxt)
      #                 values("ct24h_bdata129",l_relpamseq, 1, l_relpamtxt)
