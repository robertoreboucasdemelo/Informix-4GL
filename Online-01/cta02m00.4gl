...
#----------------------------------------------------------------------------#
# 01/11/2013  PSI-2013-23297            Alteração da utilização do sendmail  #
#----------------------------------------------------------------------------#
...
#--------------------------------------------------------------------------
 ...
 while TRUE
	...
	g_cgccpf.ligdctnum     <> 0       ) then
       ...
       ## Envio de alerta despesa com serviços acessada
       ## removido devido a inclusao de alerta após a gravacao do servico
...
#------------------------------
  ...
  if g_remocao.flag_tem > 0 then
      let msg.linha1 = "JÁ EXISTE UMA SOLICITACAO"
...
# Humberto
	...
	if g_documento.ciaempcod = 50 then
        ...
        #---> Funeral - 2008
        # se variavel empresa 14 esta com S é porque entrou no desvio
...
#=>        PSI.188425 - TRATA ASSUNTO SERV. EMERG. P/ DEPTO. ALARMES MONITORADOS
       ...
       #-------------------------------------------------------------------
       # Verifica se é assunto S11" OU "S14" OU "S53" OU "S64" OU "SUC" (PSI207233)
       #-------------------------------------------------------------------
          if d_cta02m00.c24astcod = "S11"  or
             ...
             d_cta02m00.c24astcod = "R13"  then # Assuntos Itau
	          if ctb83m00() = 0 then
	              error "Tipo de pagamento não selecionado"
	              next field c24astcod
	          end if
          end if
       ...
       if d_cta02m00.c24astcod = "W00"   or
        ...
        if l_rcuccsmtvobrflg = 'S' then
           ...
           if d_cta02m00.c24astcod = "U10" or
              ...
              # da transferencia. Desvio condicional para não abrir o popup de Motivo, ANTES de
              # Concluir na WEB.
           else
...
## CT  303208 Alberto
       ...
       if d_cta02m00.c24astcod = "W00"   and
          (lr_documento.rcuccsmtvcod = 12 or
           ...
           if ws.confirma = "S" then
              ...
              let lr_hist.mens = '"N" - Cliente não aguarda retorno do SAC'
       ...
       if ( g_documento.c24astcod = "F45"     or
          ...
          ## entendo que devemos enviar e-mail e comunicação de todas as localizações que são   #
          ## informadas na Ct24hs, mesmo que não tenhamos informação do DAF.                    #
          #######################################################################################

          ## Retorno da Regra conforme e-mail de Edson Souza e Ronigley Ferreira em 25/08/09 as 16:30
          #######################################################################################
          ##Ronigley informa:,                                                                  #
          ## Até porque o e-mail foi implementado na época com contingência, pois o sistema     #
      ...
      if g_documento.c24astcod = 'F45' then
        ...
        if l_temDaf5 = true then
          ...
          call cty13g00("",                    # Fim Integração                       |excfnl
                        ...
                        lr_cts05g00.vclcoddig, # ws.vclcoddig    # Codigo Veículo     |vclcoddig
                        lr_cts05g00.vcldes,    # Descricao Veículo                    |vcldes
                        lr_cts05g00.vcllicnum, # Placa Veículo                        |vcllicnum
                        g_documento.c24astcod, # Assunto
                        lr_cts05g00.vclchsnum, # Número Chassi                        |vclchscod
                        ...
                        lr_documento.cgccpfnum,# Número CGC/CPF                       |cgccpfnum
                        lr_documento.cgcord,   # Ordem CGC                            |cgcord
                        lr_documento.cgccpfdig,# Dígito CGC/CPF                       |cgccpfdig
                        lr_cts05g00.vclcordes, # Descrição cor Veículo                |vclcordsc
     ...
     if sqlca.sqlcode = 0 then
          ...
          if sqlca.sqlcode = 0 then
               ...
               if l_assunto = 'S60' or l_assunto = 'S63' then
                  ...
                  let l_txtsms  = "Porto Seguro Informa: ",
                                    "Para sua comodidade, os serviços à residência podem ser solicitados pelo Portal do Cliente: ",
          ...
          if sqlca.sqlcode = 0 then
               ...
               if l_assunto = 'S60' or l_assunto = 'S63' then
                  ...
                  let m_mens.subject = "Atendimento Eletrônico Porto Seguro"

                  let m_mens.msg    = "Caro Cliente, ",ascii(13),ascii(13),
                                    "A partir de agora você que tem o Porto Seguro Auto conta com mais uma facilidade. ",
                                    "Acesse http://www.333porto.com.br e solicite serviços emergenciais à residência.",
                                    ascii(13),ascii(13),
                                    "Porto Seguro Cia de Seguros Gerais"
                    ...
                    if l_assunto = 'V10' or l_assunto = 'F10' then
                       ...
                       if l_assunto = 'B50' then
                         ...
                         let m_mens.msg    = "Atendendo sua solicitação, ",ascii(13),ascii(13),
                                         "informamos que registramos seu pedido de cancelamento de seu seguro: ",l_succod, "-531-",l_aplnumdig,ascii(13),
                                         "Para efetivação do processo de cancelamento, solicitamos que contate seu corretor de seguros para que seja apresentada proposta de cancelamento de seguro.",
                                         ascii(13),ascii(13),
                                         "Porto Seguro Cia de Seguros Gerais"
                    ...
                    if l_assunto = 'B50' then
                         ...
                         if lr_gcakfilial.maides is not null then
                             ...
                             let m_mens.msg = "Caro Corretor \n\n",
                                              "O segurado ", l_segnom clipped, " entrou em contato manifestando interesse em cancelar sua apólice de seguro. \n",
                                              "Solicitamos que entre em contato com o cliente para confirmação da intenção de cancelamento e formalização do pedido. \n\n",
 ...
 define  l_mail             record
    ...
    if lr_param.vlr >= l_valor then
          let l_email.msg =  "Prezado(s), \n \n ",
                             ...
                             " de despesa com serviços e com ", lr_param.qtde using "&&" clipped, ' serviços pagos.',
                             " \n \n - Parâmetros para alerta: documentos com despesa superior ou igual a ", l_valor using "<<<<<#.##",
                             ' ou quantidade de serviços pagos superior ou igual a ',l_qtd using "&&",'. '
          let l_email.subject = "Apólice com alta despesa com serviços acessada."
          let l_envia = 1
       #else
       #   let l_email.msg =  " Prezado(s), \n \n ",
       #                        "O operador <",  g_issk.funmat using '<<<<<<<<&'," - ",cty08g00_func.funnom clipped ,">",
       ...
       #                        g_documento.ramcod    using '<<<<&'  ,"/",
       #                        g_documento.aplnumdig using '<<<<<<<<&' ,
       #                        " >,\n que tem R$ " , lr_param.vlr using "<<<<<#.##" clipped, ' de despesa com serviços.',
       #                        ' \n \n - Parâmetros para alerta: documentos com despesa superior ou igual a ', l_valor using "<<<<<#.##" ,'.'
       #   let l_email.subject = "Apólice com alta despesa com serviços acessada."
       #   let l_envia = 1
    else
