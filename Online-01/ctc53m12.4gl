...
# Modulo.........: ctc53m12                                                   # 
# Objetivo.......: Cadastro Alerta X Tipo de Assistencia                      # 
...
define ma_ctc53m12 array[04] of record
      linha char(40)            
end record
...
define l_sql char(10000)
 ...
 let l_sql = '  select cpodes,         '  
         ...
         ,  '  where cponom = ?        '     
         ,  '  order by cpocod         '     
 prepare p_ctc53m12_001 from l_sql
 declare c_ctc53m12_001 cursor for p_ctc53m12_001
 	
 let l_sql = ' select count(*)                '
          ,  ' from datkdominio               '   
          ,  ' where cponom = ?               '   
          ,  ' and   cpocod = ?               '
 prepare p_ctc53m12_002 from l_sql
 declare c_ctc53m12_002 cursor for p_ctc53m12_002
 ...
 let l_sql =  ' update datkdominio             '
           ...
           ,  ' where cponom = ?               '    
           ,  ' and   cpocod = ?               '                 
 prepare p_ctc53m12_003 from l_sql
 ...
 let l_sql =  ' insert into datkdominio   '
           ...
           ,  '   ,atlult)                '           
           ,  ' values(?,?,?,?)           '
 prepare p_ctc53m12_004 from l_sql 	
  
 
 
 let l_sql = '  delete datkdominio           '                               	
            ,'    where cponom  =  ?         '                           	                            	
 prepare p_ctc53m12_006 from l_sql
 
 
 let l_sql = ' select count(*)                '  
          ,  ' from datkdominio               '  
          ,  ' where cponom = ?               '  
 prepare p_ctc53m12_007 from l_sql               
 declare c_ctc53m12_007 cursor for p_ctc53m12_007 
...
define lr_retorno record
 ...
 for  arr_aux  =  1  to  04                  
    initialize  ma_ctc53m12[arr_aux].* to  null  
 end  for                                     
 
 
 initialize mr_ctc53m12.*, lr_retorno.* to null 
 
 let arr_aux = 1
 
 if m_prepare is null or
    m_prepare <> true then
    call ctc53m12_prepare()
 end if
    
 let m_chave = ctc53m12_monta_chave(mr_param.codper    , 
                                    mr_param.c24astcod ,
                                    mr_param.clscod    ,
                                    mr_param.asitipcod )
 ...
 open c_ctc53m12_001  using  m_chave                           
 foreach c_ctc53m12_001 into ma_ctc53m12[arr_aux].linha,       
 	                           mr_ctc53m12.atldat        ,        
                             mr_ctc53m12.funmat                
 ...
 let mr_param.linha1 = ma_ctc53m12[1].linha
 let mr_param.linha2 = ma_ctc53m12[2].linha
 let mr_param.linha3 = ma_ctc53m12[3].linha
 let mr_param.linha4 = ma_ctc53m12[4].linha
 

 open window w_ctc53m12 at 6,2 with form 'ctc53m12'
 attribute(form line 1)
  
 let mr_ctc53m12.msg =  '                      (F17)Abandona,(F2)Exclui'
 ...
 input by name mr_param.linha1, 
      ...
      #---------------------------------------------                
        ...
        if fgl_lastkey() = fgl_keyval ("up")     or        
         ...
         if m_operacao <> 'i' then                         
            ...
            #--------------------------------------------------------
            # Atualiza Associacao Alerta X Tipo         
            #--------------------------------------------------------
            
            call ctc53m12_altera()
            next field linha1                              
      ...
      #---------------------------------------------                     
         ...
         #-------------------------------------------------------- 
         # Exclui Associacao Alerta X Tipo de Assistencia                  
         #-------------------------------------------------------- 
         
         if not ctc53m12_delete() then       	
...
initialize lr_retorno.* to null                                                                                               
  ...
  call cts08g01("A","S"                                                                        
     ...
     if lr_retorno.confirma  = "S" then                                                                  
        ...
        execute p_ctc53m12_006 using m_chave
...
initialize lr_retorno.* to null           
    ...
    let ma_ctc53m12[1].linha = mr_param.linha1
    let ma_ctc53m12[2].linha = mr_param.linha2
    let ma_ctc53m12[3].linha = mr_param.linha3
    let ma_ctc53m12[4].linha = mr_param.linha4
    
       
    for  l_arr_aux  =  1  to  04
    	 
         whenever error continue 
         execute p_ctc53m12_003 using ma_ctc53m12[l_arr_aux].linha   
...
initialize lr_retorno.* to null                                     
    ...
    for  l_arr_aux  =  1  to  04
        ...
        whenever error continue
        execute p_ctc53m12_004 using l_arr_aux  
                                   , ma_ctc53m12[l_arr_aux].linha
                                   , m_chave
                                   , lr_retorno.atlult
...
#===============================================================================     
 function ctc53m12_monta_chave(lr_param)                                                         
#=============================================================================== 
...
initialize lr_retorno.* to null
 ...
 let lr_retorno.chave = "PF_ALE", lr_param.codper using "&&"
...
#---------------------------------------------------------           
 function ctc53m12_valida_exclusao(lr_param)                         
#---------------------------------------------------------           
...
if m_prepare is null or                                              
   ...
   let m_chave = ctc53m12_monta_chave(lr_param.codper   ,            
                                      lr_param.c24astcod,            
                                      lr_param.clscod   ,            
                                      lr_param.asitipcod)            
                                                                     
   open c_ctc53m12_007 using m_chave                                 
                                                                     
   whenever error continue                                           
   fetch c_ctc53m12_007 into  lr_retorno.cont                        
   whenever error stop                                               
