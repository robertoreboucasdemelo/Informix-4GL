...
# Modulo.........: cts11m13                                                  #   
# Objetivo.......: Lista de Serviços Originais do Itau                       #   
...
define l_sql char(10000)
  ...
  let l_sql = "select b.c24astcod,       " ,
              ...
              " and c.itaasiplncod = ?          " ,  
              " and c.itaasiplntipcod in (5,7)  " ,                                                 
              " and d.itaciacod = ?             " ,  
              " and d.itaramcod = ?             " ,  
              " and d.itaaplnum = ?             " ,  
              " and d.itaaplitmnum = ?          "    
  prepare p_cts11m13_001  from l_sql                 
  declare c_cts11m13_001  cursor for p_cts11m13_001  


end function   

#------------------------------------------------------------------------------  
function cts11m13_rec_servico_original(lr_param)                                         
#------------------------------------------------------------------------------  
...
if m_prepare is null or                                                          
   m_prepare <> true then                                                        
   call cts11m13_prepare()                                                       
end if                                                                           
                                                                                 
   let l_index = 1                                                               
                                                                                 
   open c_cts11m13_001 using lr_param.itaaplvigincdat , 
   ...
   foreach c_cts11m13_001 into mr_retorno[l_index].c24astcod  ,                
...
define a_cts11m13    array[500] of record
   assunto           char(3)                    ,
   servico           char (13)                  ,
   atddat            like datmservico.atddat    ,
   atdhor            like  datmservico.atdhor   ,   
...
define l_index     integer  ,
 ...
 # inicializando array tela servicos
   for l_index  =  1  to  500
       initialize  a_cts11m13[l_index].* to  null       
   end for 
 
   call cts11m13_rec_servico_original(lr_param.itaasiplncod   ,
 ...
 if l_qtde = 0 then 
  ...
  for l_index = 1 to l_qtde 
     ...
     let a_cts11m13[l_index].assunto = mr_retorno[l_index].c24astcod
     let a_cts11m13[l_index].servico = mr_retorno[l_index].atdsrvorg using "&&",
                                       "/", mr_retorno[l_index].atdsrvnum using "&&&&&&&",
                                       "-", mr_retorno[l_index].atdsrvano using "&&"
     let a_cts11m13[l_index].atdhor     = mr_retorno[l_index].atdhor
     let a_cts11m13[l_index].atddat     = mr_retorno[l_index].atddat
     let a_cts11m13[l_index].c24solnom  = mr_retorno[l_index].c24solnom         
  ...
  if arr_aux  =  0   then
     ...
     open window w_cts11m13 at 11,10  with form  "cts11m13"
     ...
     display array a_cts11m13 to s_cts11m13.*
        on key(interrupt)
        ...
        on key(F8)
           ...
           let lr_retorno.atdsrvorg = a_cts11m13[arr_aux].servico[01,02]
           let lr_retorno.atdsrvnum = a_cts11m13[arr_aux].servico[04,10]
           let lr_retorno.atdsrvano = a_cts11m13[arr_aux].servico[12,13]
                  
           if a_cts11m13[arr_aux].assunto = g_documento.c24astcod then 
              call cts08g01("A","S",
