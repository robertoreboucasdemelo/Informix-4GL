...
#   { &wissc219_mensagem($titgif,$cabec,"LOGIN","Código do padrão não informado!"); }
 ...
 #-----------------------------------------------------------------------------
 $prog = "(cd /webserver/cgi-bin/ct24h/trs ; . ../../setvar.sh; wctn03m01.4gc $ARG 2>&1)";
 ...
 start_form(-method=>'POST',
            -name=>'laudo',
            -action=>"/cgi-bin/ct24h/trs/wctn03m05.pl?usrtip=%s&webusrcod=%d&sesnum=%d&macsissgl=%satdsrvnum=%d&atdsrvano=%d&atdetpseq=%d&,$usrtip,$webusrcod,$sesnum,$macsissgl,$atdsrvnum,$atdsrvano,$atdetpseq"),"\n", 
...
#push @botoes,
#     submit (-value=>' Aceitar ', -name=>'Acao'),
#     submit (-value=>'Recusar', -name=>'Acao');   
