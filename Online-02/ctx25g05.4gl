...
#-----------------------------#
  ...
  while true
     ...
     if l_sede = true then
        ...
        call ctx25g05_pesquisa_cep(lr_parametro.lgdtip,
                               lr_input.lgdnom,
                               lr_input.lgdnum,
                               lr_parametro.brrnom,
                               lr_input.ufdcod,
...
#--------------------------------------#
  ...
  define l_status          integer, # 0 -> OK     <> 0 -> ERRO
...
#------------------------------------------#
function ctx25g05_pesquisa_cep(lr_parametro)
#------------------------------------------#
  ...
  while true
     ...
     if l_status <> 0 then
        ...
        # Formata CEP
        if lr_parametro.lgdcep > 0 and
     	     lr_parametro.lgdcepcmp > 0 then
     	     let l_cepcod = lr_parametro.lgdcep using "&&&&&", lr_parametro.lgdcepcmp using "&&&"
        else
     	     initialize l_cepcod to null
        end if
        ...
        call ctx25g05_processa_enderecos_xml(l_qtd_end,
                                             ...
                                             l_cepcod           ,   # Envia CEP para priorizar apresentacao
                                             lr_fonetica.prifoncod) # Envia codigo fonetico para priorizar apresentacao
...
#---------------------------------------#
 ...
 # Delega execucao para a nova funcao ctx25g05_pesq_auto_cep
 call ctx25g05_pesq_auto_cep(lr_parametro.lgdtip,
                             lr_parametro.lgdnom,
                             lr_parametro.lgdnum,
                             lr_parametro.brrnom,
                             lr_parametro.lclbrrnom,
...
#-------------------------------------------#
function ctx25g05_pesq_auto_cep(lr_parametro)
#-------------------------------------------#
    ...
    if l_status <> 0 then
       ...
       if m_qtd_end > 0 then
          ...
          if sqlca.sqlcode = 0 then
              ...
              display "ERRO sqlca.sqlcode: ", sqlca.sqlcode
          end if
...
#----------------------------------------------------#
  ...
  for l_ind = 1 to lr_parametro.qtd_end
     ...
     if  not m_flgref then
         ...
         let lr_auxiliar.prioridade = 14

         if m_brrnom = lr_parametro.brrnom then
            let lr_auxiliar.prioridade = 13
         end if

         if m_lgdnom = lr_parametro.lgdnom then
            let lr_auxiliar.prioridade = 12
         end if

         if m_tipendtxt = lr_auxiliar.tipendtxt then
            let lr_auxiliar.prioridade = 11
         end if

         if m_lgdnom = lr_parametro.lgdnom and
         	  m_brrnom = lr_parametro.brrnom then
            let lr_auxiliar.prioridade = 10
         end if

         # BATE cidade, bairro e CEP
         if m_brrnom = lr_parametro.brrnom and
         	  m_cep = lr_parametro.cepcod
         	  then
            let lr_auxiliar.prioridade = 7
         end if

         # BATE cidade, logradouro e CEP
...
#------------------------------------------------#
 ...
 define l_mpalgdincnum like datkmpalgdsgm.mpalgdincnum
      ...
      , l_cidnom       char(45)
      , l_foneticaigual smallint
      , l_cidadeigual smallint
      , l_cepigual smallint
      , l_numeracaovalida smallint

 initialize l_mpacidcod, l_lgdtip, l_lgdnom, l_brrnom, l_cepcod, l_lclltt, l_lcllgt
          , l_mpalgdincnum, l_mpalgdfnlnum, l_cidnom, l_foneticaigual, l_cidadeigual
          , l_cepigual, l_numeracaovalida to null
 ...
 let l_sql = "select ",
                   ...
                   " sgm.mpalgdincnum, ",
                   " sgm.mpalgdfnlnum, ",
                   " case when lgd.prifoncod = '", lr_parametro.prifoncod clipped, "' then 1 else 0 end FoneticaIgual, ",
                   " case when lgd.mpacidcod = ", l_mpacidcod, " then 1 else 0 end CidadeIgual, "
                   if lr_parametro.cepcod is null then
                      let l_sql = l_sql clipped,
                                  " 0 CEPIgual, "
                   else
                      let l_sql = l_sql clipped,
                                  " case when sgm.cepcod    = ", lr_parametro.cepcod, " then 1 else 0 end CEPIgual, "
                   end if
                   let l_sql = l_sql clipped,
                   " case when (sgm.mpalgdincnum <= ", lr_parametro.lgdnum, "  and sgm.mpalgdfnlnum >= ", lr_parametro.lgdnum, " ) then 1 else 0 end NumeracaoValida ",
              " from datkmpalgd lgd, ",
                   " datkmpalgdsgm sgm, ",
                   " outer datkmpabrr brr ",
             " where lgd.mpacidcod = ", l_mpacidcod, " and lgd.lgdnom like '", l_nompsq, "'",
             ...
             "select ",
              ...
              " from datkmpalgd lgd, ",
                   " datkmpalgdsgm sgm, ",
                   " outer datkmpabrr brr ",
             " where lgd.mpacidcod = ", l_mpacidcod, " and lgd.prifoncod = '", lr_parametro.prifoncod clipped, "' ",
               " and sgm.mpalgdcod = lgd.mpalgdcod ",
               " and brr.mpabrrcod = sgm.mpabrrcod "
          if lr_parametro.cepcod is not null then
             let l_sql = l_sql clipped,
             " union ",
             "select ",
                   " lgd.lgdtip, ",
              ...
              " from datkmpalgd lgd, ",
                   " datkmpalgdsgm sgm, ",
                   " outer datkmpabrr brr ",
             " where sgm.cepcod is not null and sgm.cepcod = ", lr_parametro.cepcod, " ",
               " and sgm.mpalgdcod = lgd.mpalgdcod ",
               " and brr.mpabrrcod = sgm.mpabrrcod "
          end if
      let l_sql = l_sql clipped,
      " order by CidadeIgual desc, ",    # MESMA CIDADE
               " CEPIgual desc, ",       # MESMO CEP
               " FoneticaIgual desc, ",  # MESMA FONETICA
               " NumeracaoValida desc " # DENTRO DA NUMERACAO

 prepare pctx25g05011 from l_sql
 declare cctx25g05011 cursor for pctx25g05011
 ...
 foreach cctx25g05011 into l_lgdtip,
     ...
     let m_lgdfnlnum = l_mpalgdfnlnum
         ...
         # BATE CEP, logradouro fonetica e bairro
         if l_cidadeigual and
         	  m_lgdtip = lr_parametro.lgdtip and l_foneticaigual and
         	  m_brrnom = lr_parametro.brrnom  and
         	  l_numeracaovalida
         	  then
            let lr_auxiliar.prioridade = 9
         end if

         # BATE CEP, logradouro e bairro
         if l_cidadeigual and
         	  m_tipendtxt = lr_auxiliar.tipendtxt and
         	  m_brrnom = lr_parametro.brrnom
         	  then
            let lr_auxiliar.prioridade = 8
         end if
         ...
         # BATE cidade, logradouro fonetica e CEP
         if l_cidadeigual and
            m_lgdtip[1] = lr_parametro.lgdtip[1] and l_foneticaigual and
         	  l_cepigual
         	  then
         ...
         # BATE cidade, logradouro fonetica e bairro
         if l_cidadeigual and
         	  m_lgdtip[1] = lr_parametro.lgdtip[1] and l_foneticaigual and
         	  m_brrnom = lr_parametro.brrnom
         	  then
         ...
         # BATE cidade, logradouro e bairro
         if l_cidadeigual and
         	  m_tipendtxt = lr_auxiliar.tipendtxt and
         	  m_brrnom = lr_parametro.brrnom
         	  then
         ...
         # BATE cidade, logradouro fonetica, bairro e numeracao
         if l_cidadeigual and
         	  m_lgdtip[1] = lr_parametro.lgdtip[1] and l_foneticaigual and
         	  m_brrnom = lr_parametro.brrnom  and
         	  l_numeracaovalida
         ...
         # BATE cidade, logradouro, bairro e numeracao
         if l_cidadeigual and
         	  m_tipendtxt = lr_auxiliar.tipendtxt and
         	  m_brrnom = lr_parametro.brrnom  and
         	  l_numeracaovalida
